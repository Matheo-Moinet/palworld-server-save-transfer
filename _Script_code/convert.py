...
def read_and_convert_sav_to_json(
    filename, force=False, minify=False, allow_nan=True
):
    print(f"Decompressing sav file")
    with open(filename, "rb") as f:
        data = f.read()
        raw_gvas, _ = decompress_sav_to_gvas(data)
    print(f"Loading GVAS file (can take a while...)")
    gvas_file = GvasFile.read(
        raw_gvas, PALWORLD_TYPE_HINTS, PALWORLD_CUSTOM_PROPERTIES, allow_nan=allow_nan
    )
    ...
    js = json.loads(json.dumps(
        gvas_file.dump(), indent=indent, cls=CustomEncoder, allow_nan=allow_nan
    ))
    return js
    print(js["properties"]["SaveData"]["value"]["IndividualId"]["value"]["PlayerUId"]["value"])
...
def write_and_convert_jsons_to_sav(jsons, output_path, force=False):
    if os.path.exists(output_path):
        print(f"{output_path} already exists, this will overwrite the file")
        if not force:
            if not confirm_prompt("Are you sure you want to continue?"):
    ...
    #data = json.dumps(jsons,  indent='\t', cls=CustomEncoder, allow_nan=True)
